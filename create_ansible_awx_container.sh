...
pct create $VMID $OSTEMPLATE --rootfs $DISKSIZE --swap $SWAPSIZE --cores $CPUS --memory $MEMORY --net0 name=eth0,ip=$IPADDRESS/24,gw=$GATEWAY --nameserver $DNS
...
echo "AWX should now be installed and accessible at https://$IPADDRESS with your root password."
